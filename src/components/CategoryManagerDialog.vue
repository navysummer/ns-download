...
<template>
  ...
  >
    <div :style="{ backgroundColor: '#261C14', border: '1px solid #5A4330', borderRadius: '12px', width: '420px', maxHeight: '520px' }"
    ...
    >
      <div class="flex items-center justify-between px-4 py-3" :style="{ borderBottom: '1px solid #5A4330' }">
        <span class="text-sm font-semibold" :style="{ color: '#EDE0C8' }">分类管理</span>
      ...
      <div class="flex-1 overflow-y-auto px-3 py-2">
        ...
        >
          <File class="h-3.5 w-3.5 shrink-0 cursor-pointer" :style="{ color: cat.visible ? 'var(--accent)' : '#9C8260' }" @click="store.toggleCategoryVisibility(cat.id)" />
          <template v-if="editingId === cat.id">
            <div class="flex-1 space-y-1">
              ...
              <input v-model="editExts" @keyup.enter="saveEdit(cat.id)"
                ...
                :style="{ color: '#C9B393', border: '1px solid #5A4330' }"
                placeholder="扩展名，用逗号分隔"
              />
            </div>
            <button @click="saveEdit(cat.id)" class="rounded p-1" :style="{ color: '#4E7A5A' }"><Check class="h-3.5 w-3.5" /></button>
          </template>
          <template v-else>
            <span class="flex-1 text-sm" :style="{ color: cat.visible ? '#EDE0C8' : '#9C8260' }">{{ cat.name }}</span>
            <span class="text-2xs" :style="{ color: '#9C8260' }">{{ cat.extensions.length }} 个扩展名</span>
            <button v-if="!cat.isBuiltin" @click="startEdit(cat.id)" class="rounded p-1" :style="{ color: '#9C8260' }"><Edit3 class="h-3.5 w-3.5" /></button>
            <button v-if="!cat.isBuiltin" @click="store.removeCategory(cat.id)" class="rounded p-1" :style="{ color: '#B53A2E' }"><Trash2 class="h-3.5 w-3.5" /></button>
          </template>
        </div>
      </div>

      <div class="space-y-1.5 px-3 py-2" :style="{ borderTop: '1px solid #5A4330' }">
