  ...
  <footer :style="{ backgroundColor: '#2C2C2E', borderTop: '1px solid #48484A', color: '#8E8E93', height: '28px' }"
    ...
    <div class="flex items-center gap-4">
      ...
      <div class="flex items-center gap-1">
        ...
        <span class="tabular-nums">{{ formatSpeed(0) }}/s</span>
      </div>
    ...
    <div class="flex items-center gap-3">
      ...
      <div class="relative">
        <button @click="showSpeedPopover = !showSpeedPopover"
          ...
          :style="{ color: isLimited ? '#3B82F6' : '#8E8E93' }"
        >
          <Gauge class="h-2.5 w-2.5" />
          <span class="tabular-nums">{{ isLimited ? `${limitKbs} KB/s` : '无限' }}</span>
        </button>
        ...
        >
          <div class="p-3">
            <div class="mb-2 flex items-center justify-between">
              ...
              <button @click="isLimited = !isLimited"
                class="rounded-sm px-2 py-0.5 text-2xs font-medium transition-colors"
                :style="{
                  backgroundColor: isLimited ? '#3B82F6' : '#3A3A3C',
                  color: isLimited ? '#fff' : '#A1A1A6',
                }"
              >{{ isLimited ? '开启' : '关闭' }}</button>
            </div>
            <div class="flex flex-wrap gap-1.5">
              <button v-for="p in speedPresets" :key="p.kbs"
                @click="isLimited = true; limitKbs = p.kbs"
                class="rounded px-2 py-1 text-2xs transition-colors"
                :style="{
                  backgroundColor: isLimited && limitKbs === p.kbs ? '#3B82F6' : '#3A3A3C',
                  color: isLimited && limitKbs === p.kbs ? '#fff' : '#A1A1A6',
                  border: isLimited && limitKbs === p.kbs ? '1px solid #3B82F6' : '1px solid #48484A',
                }"
              >{{ p.label }}</button>
            </div>
