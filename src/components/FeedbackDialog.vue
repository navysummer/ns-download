    ...
    >
      ...
      <template v-if="sent">
        <div class="flex flex-col items-center justify-center py-8 gap-2">
          <span class="text-lg" :style="{ color: 'var(--success)' }">✓</span>
          <span class="text-sm font-medium" :style="{ color: 'var(--text-primary)' }">感谢您的反馈！</span>
          <span class="text-xs" :style="{ color: 'var(--text-muted)' }">我们会在后续版本中考虑您的建议</span>
        </div>
      </template>
      <template v-else>
        <div class="px-4 py-4 space-y-3">
          <div>
            <span class="text-xs font-medium" :style="{ color: 'var(--text-secondary)' }">类型</span>
            <div class="flex gap-2 mt-1">
              <button v-for="t in [{id:'bug',label:'Bug'},{id:'suggestion',label:'建议'},{id:'other',label:'其他'}]" :key="t.id"
                ...
                :style="{
                  backgroundColor: feedbackType === t.id ? 'var(--accent)' : 'var(--surface-bg2)',
                  color: feedbackType === t.id ? 'var(--text-primary)' : 'var(--text-secondary)',
                }"
              >{{ t.label }}</button>
            </div>
          </div>
          <div>
            <span class="text-xs font-medium" :style="{ color: 'var(--text-secondary)' }">内容</span>
          ...
          <div>
            <span class="text-xs font-medium" :style="{ color: 'var(--text-secondary)' }">联系方式 (可选)</span>
        ...
        <div class="flex justify-end px-4 py-3" :style="{ borderTop: '1px solid var(--surface-border)' }">
