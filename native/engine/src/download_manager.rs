...
impl DownloadManager {
    ...
    async fn plugin_match_resolver(&self, url: &str) -> String {
        match &self.plugin_manager {
            Some(pm) => pm.match_resolver(url).unwrap_or_default(),
            None => String::new(),
        }
    }
    ...
    /// ```no_run
    /// # use std::sync::Arc;
    /// # use ns_download_engine::{Engine, EngineConfig, NoopSelection, NoopSink};
    /// # use ns_download_engine::bt_downloader::BtConfig;
    /// # use ns_download_engine::proxy_config::ProxyConfig;
    /// # async fn run() -> Result<(), ns_download_engine::EngineError> {
    /// # let config = EngineConfig { max_concurrent: 5, speed_limit_bps: 0, default_save_dir: "/tmp/downloads".to_string(), app_data_dir: "/tmp/nsdownload".to_string(), bt_config: BtConfig::default(), proxy_config: ProxyConfig::default(), user_agent: String::new(), data_dir_override: None, database_url: None };
    ...
    async fn do_start_task(&mut self, queued: QueuedTask) {
        ...
        let handle = if use_bt {
            ...
            let params = DownloadParams {
                ...
                ffmpeg_path: crate::components::resolve_ffmpeg(&self.data_dir).await,
            };
...
mod tests {
    ...
    // -------------------------------------------------------------------------
    // 文件跟踪（NSDownload #11）：task_target_path / probe_missing / scan_missing_files
    // -------------------------------------------------------------------------

    /// NSDownload #11：空名与路径穿越/绝对路径必须解析为 `None`——无法安全判定
    ...
    fn unique_filetrack_test_dir(tag: &str) -> PathBuf {
        std::env::temp_dir().join(format!(
            "nsdownload_filetrack_test_{tag}_{}_{}",
            std::process::id(),
    ...
    /// NSDownload #11 核心契约：completed 任务的目标文件消失后 `file_missing`
